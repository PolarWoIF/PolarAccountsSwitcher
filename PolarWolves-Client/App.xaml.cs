...
using PolarWolves_Server.Pages.Basic;
using PolarWolves_Server.Pages.General;
using PolarWolves_Server.Pages.Steam;
using static PolarWolves_Client.MainWindow;
...
#pragma warning restore CA2211 // Non-constant fields should not be visible
        ...
        protected override async void OnStartup(StartupEventArgs e)
        {
            ...
            if (e.Args.Length != 0) // An argument was passed
            {
                ...
                // - But, do check if a protocol command was issued, and only that - If it was: start the program.
                if (StartPage == "" && !(Globals.VerboseMode && e.Args.Length == 1) && !(e.Args.Length == 1 &&
                    (e.Args[0] == @"polarwolves:\\" || e.Args[0] == @"polarwolves:\\" || e.Args[0] == "polarwolves:\\" || e.Args[0] == "polarwolves:\\")))
                {
            ...
            {
                MessageBox.Show("Can't access logs in the PolarWolves directory!",
                ...
                (Keyboard.GetKeyStates(Key.Scroll) & KeyStates.Down) != 0)
            {
                ...
                if (GeneralFuncs.CanKillProcess("PolarWolves"))
                    Globals.KillProcess("PolarWolves");
            }
            ...
            if (Directory.Exists("newUpdater"))
            {
                ...
                catch (IOException)
                {
                    ...
                    Globals.KillProcess("PolarWolves-Updater");
                    Globals.RecursiveDelete("updater", false);
                }
...
release = true;
                ...
                if (!NativeFuncs.BringToFront())
                {
	                text = "Another PolarWolves instance has been detected." + Environment.NewLine +
	                       "[Something wrong? Hold Hold Alt, Ctrl, Shift or Scroll Lock while starting to close all PolarWolves processes!]";

                    _ = MessageBox.Show(text, "PolarWolves Notice", MessageBoxButton.OK,
                ...
                else
                {
	                ...
	                {
		                text = "PolarWolves was running." + Environment.NewLine +
		                       ...
		                       "[Something wrong? Hold Alt, Ctrl, Shift or Scroll Lock while starting to close all PolarWolves processes!]" + Environment.NewLine + Environment.NewLine +
        ...
        /// <summary>
        /// CLI specific interface for PolarWolves
        ...
        {
            Console.WriteLine(@"Welcome to the PolarWolves - Command Line Interface!");
            ...
            for (var i = 0; i != e.Args.Length; ++i)
            {
                ...
                if (e.Args[i].ToLowerInvariant().StartsWith(@"polarwolves:\\") || e.Args[i].ToLowerInvariant().StartsWith(@"polarwolves:\\"))
                {
                    CliSwitch(e.Args, i);
                    continue;
                }
        ...
        private static void CliSwitch(string[] args, int i)
        {
            ...
            if (args[i].StartsWith(@"polarwolves:\\", StringComparison.InvariantCultureIgnoreCase)) // Launched through Protocol
                args[i] = '+' + args[i][14..];
            else if (args[i].StartsWith(@"polarwolves:\\", StringComparison.InvariantCultureIgnoreCase))
                args[i] = '+' + args[i][7..];
                ...
                case "s":
                    {
                        ...
                        if (!GeneralFuncs.CanKillProcess(PolarWolves_Server.Data.Settings.Steam.Processes)) Restart(combinedArgs, true);
