...
// using Process.Start() the process is started as a child, and ends when PolarWolves closes.
...
if (args.Length > 0)
    WriteLog("Arguments received (values redacted).");
...
try
{
    ...
    WriteLog($"Starting process: FileName='{filePath}', Elevated={isElevated}, WorkingDirectory='{targetDir}'");
