...
namespace PolarWolves_Globals
{
    internal static class NativeMethods
    {
        ...
        // PolarWolves-Client NativeMethods
    ...
    public static class NativeFuncs
    {
        ...
        public static bool BringToFront()
        {
            ...
            var proc = Process.GetProcessesByName("PolarWolves_main").FirstOrDefault();
        ...
        {
            if (Process.GetProcessesByName("PolarWolves-Tray_main").Length > 0) return "Already running";
            if (!File.Exists("Tray_Users.json")) return "Tray users not found";
            var startInfo = new ProcessStartInfo { FileName = Path.Join(Globals.AppDataFolder, "PolarWolves-Tray.exe"), CreateNoWindow = false, UseShellExecute = false, WorkingDirectory = Globals.AppDataFolder };
            try
            {
