...
#pragma warning restore CA2211 // Non-constant fields should not be visible
        ...
        private void LaunchAccSwitcher(object sender, RoutedEventArgs e)
        {
            try
            {
                _ = Process.Start(new ProcessStartInfo(@"PolarWolves.exe") { UseShellExecute = true });
                Process.GetCurrentProcess().Kill();
            }
            catch (Win32Exception we)
            {
                MessageBox.Show(
                    $"An error occurred while trying to start. Patching may have failed. Try verify using this updater, or re-download a fresh copy from the PolarWolves GitHub.{Environment.NewLine}{we}",
                    "Could not start PolarWolves", MessageBoxButton.OK, MessageBoxImage.Error);


                SetStatus("An error occurred while trying to start. Patching may have failed. Try verify using this updater, or re-download a fresh copy from the PolarWolves GitHub.");
        ...
        public static void RestartAsAdmin(string args)
        {
            var proc = new ProcessStartInfo
            {
                ...
                FileName = Assembly.GetEntryAssembly()?.Location.Replace(".dll", ".exe") ?? "PolarWolves.exe",
        ...
        private void Init()
        {
            ...
            {
                MessageBox.Show("Please run the main program at least once, with 'PolarWolves.exe'", "Error", MessageBoxButton.OK, MessageBoxImage.Warning);
        ...
        private static string GetVersionFromGlobals()
        {
            ...
            var dllPath = Path.Join(Directory.GetCurrentDirectory(), "PolarWolves-Globals.dll");
            ...
            try
            {
                ...
                var version = Assembly.LoadFrom(tempDll).GetType("PolarWolves_Globals.Globals")
        ...
        {
            const string newFolder = "PolarWolves";
...
#if DEBUG
            latestAvailable = client.DownloadString(new Uri("https://discord.gg/PLR/Projects/AccSwitcher/api?debug&v=" +
                                              _currentVersion));
#else
             latestAvailable = client.DownloadString(new Uri("https://discord.gg/PLR/Projects/AccSwitcher/api?v=" +
                                                          _currentVersion));
#endif
            DoUpdate(cef: true);
        }
        ...
        // FOR TESTING: public static string MainAppDataFolder => "C:\\Program Files\\PolarWolves";
        ...
        private void DoUpdate(bool cef = false)
        {
            ...
            SetStatusAndLog("Closing PolarWolves instances (if any).");
            ...
            {
                var downloadUrl = "";
                if (cef) downloadUrl = $"https://discord.gg/PLR/releases/download/{_latestVersion}/PolarWolves_and_CEF_{_latestVersion}.7z";
                else downloadUrl = $"https://discord.gg/PLR/releases/download/{_latestVersion}/PolarWolves_{_latestVersion}.7z";
            ...
            } catch (Exception e)
            {
                ...
                WriteLine("Should issues persist please contact PolarWolves Team with this info.");
                return;
            }
            ...
            var tempFolder = Path.Combine(Path.GetTempPath(), "PolarWolves");
        ...
        private static bool usingCEF()
        {
            var path = File.Exists(Path.Join(AppDataFolder, "userdata_path.txt"))
                ...
                : Path.Join(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "PolarWolves\\");
        ...
        /// <summary>
        /// Gets list of updates from https://discord.gg/PLR
        /// </summary>
        ...
        private void GetUpdatesList(ref Dictionary<string, string> updatesAndChanges)
        {
...
#if DEBUG
            var versions =
                client.DownloadString(new Uri("https://discord.gg/PLR/Projects/AccSwitcher/api/update?debug&v=" +
                                              _currentVersion));
            latestAvailable = client.DownloadString(new Uri("https://discord.gg/PLR/Projects/AccSwitcher/api?debug&v=" +
                                              _currentVersion));
#else
            var versions = client.DownloadString(new Uri("https://discord.gg/PLR/Projects/AccSwitcher/api/update?v=" + _currentVersion));
             latestAvailable = client.DownloadString(new Uri("https://discord.gg/PLR/Projects/AccSwitcher/api?v=" +
                                                          _currentVersion));
#endif
            try
            ...
            {
                MessageBox.Show(
                    "This is most likely due to me (PolarWolves Team) pushing an update, and making a typo that broke the .json update file. Do let me know :)",
        ...
        /// <summary>
        /// Closes PolarWolves's processes if any are running. (Searches program's folder recursively)
        /// </summary>
        ...
        private void CloseIfRunning(string currentDir)
        {
            foreach (var exe in Directory.GetFiles(currentDir, "*.exe", SearchOption.AllDirectories))
            {
                ...
                try
                {
                    if (Process.GetProcessesByName(exe).Length <= 0 || exe == "PolarWolves-Updater.exe" || exe == "PolarWolves-Updater_main.exe") continue;
        ...
        /// <summary>
        /// Used by PolarWolves Team to create updates for the program.
        /// </summary>
