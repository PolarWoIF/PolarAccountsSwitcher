...
async function getCurrentPageFullname() {
    ...
    if (platform === "Basic") {
        platform = await DotNet.invokeMethodAsync("PolarWolves-Server", "GiCurrentBasicPlatform", platform);
    }
    return platform;
}
...
function initPlatformCardsMotion() {
    ...
    cards.forEach((card, index) => {
        ...
        const resetCardVisuals = () => {
            ...
            card.style.setProperty("--card-sheen-shift", "-30%");
        ...
        const applyCardVisuals = () => {
            ...
            const distance = Math.min(1, Math.hypot(dx, dy) * 2.1);

            const tiltX = (0.5 - state.currentY) * 7.2;
            const tiltY = (state.currentX - 0.5) * 8.2;
            const glow = state.hovering ? (0.11 + (distance * 0.2)) : (distance * 0.05);
            const sheenShift = state.hovering
                ? (((state.currentX - 0.5) * 22) - 8)
                : (((state.currentX - 0.5) * 8) - 30);

            card.style.setProperty("--card-tilt-x", `${tiltX.toFixed(2)}deg`);
            card.style.setProperty("--card-tilt-y", `${tiltY.toFixed(2)}deg`);
            card.style.setProperty("--card-mx", `${(state.currentX * 100).toFixed(2)}%`);
            card.style.setProperty("--card-my", `${(state.currentY * 100).toFixed(2)}%`);
            card.style.setProperty("--card-glow", glow.toFixed(3));
            card.style.setProperty("--card-sheen-shift", `${sheenShift.toFixed(2)}%`);
        };

        const animate = () => {
            const easing = state.hovering ? 0.24 : 0.18;
            state.currentX += (state.targetX - state.currentX) * easing;
            state.currentY += (state.targetY - state.currentY) * easing;
            applyCardVisuals();

            const settledX = Math.abs(state.targetX - state.currentX) < 0.002;
            const settledY = Math.abs(state.targetY - state.currentY) < 0.002;
            const settled = settledX && settledY;
...
function filterHomePlatforms(query) {
    const normalized = (query || "").trim().toUpperCase();
    const cards = document.querySelectorAll(".main-platform-rework.zenith-home .platform_list_item");
    cards.forEach((card) => {
        const fallback = card.querySelector(".fgText p");
        const cardName = ((card.dataset.homeName || (fallback ? fallback.textContent : "")) || "").toUpperCase();
        const show = normalized === "" || cardName.indexOf(normalized) !== -1;
        card.classList.toggle("is-search-hidden", !show);
    });
}

window.filterHomePlatforms = filterHomePlatforms;
...
GetLang = async(k) => await DotNet.invokeMethodAsync("PolarWolves-Server", "GiLocale", k);
GetCrowdin = async() => await DotNet.invokeMethodAsync("PolarWolves-Server", "GiCrowdinList");
GetLangSub = async(key, obj) => await DotNet.invokeMethodAsync("PolarWolves-Server", "GiLocaleObj", key, obj);

copyToClipboard = async(str) => await DotNet.invokeMethodAsync("PolarWolves-Server", "CopyToClipboard", str);

// FORGETTING ACCOUNTS
async function forget(e) {
    e.preventDefault();
    const reqId = $(selectedElem).attr("id");
    const pageForgetAcc = await DotNet.invokeMethodAsync("PolarWolves-Server", `Get${getCurrentPage()}ForgetAcc`);
...
getAccNotes = async(accId) => await DotNet.invokeMethodAsync("PolarWolves-Server", `Get${getCurrentPage()}Notes`, accId);
...
async function copy(request, e) {
    ...
    async function steam() {
        ...
        switch (request) {
            ...
            case "SteamId3":
            case "SteamId":
                await DotNet.invokeMethodAsync("PolarWolves-Server", "CopySteamIdType", request, steamId64);
                break;
...
function getSelectedDisplayName() {
    ...
    const displayName = selected.next("label").find(".displayName").first().text();
    if (displayName && displayName.trim() !== "") return displayName.trim();

    const attrDisplayName = selected.attr("displayname") || selected.attr("DisplayName");
    return (attrDisplayName ?? "").trim();
}

function formatPlaytime(playtimeMinutes) {
    const minutes = Number(playtimeMinutes) || 0;
...
async function swapTo(request, e) {
    ...
    if (typeof animateAccountSwitchAction === "function") {
        animateAccountSwitchAction(selected.attr("id"));
    }

    if (request === -1) await DotNet.invokeMethodAsync("PolarWolves-Server", `SwapTo${getCurrentPage()}`, selected.attr("id")); // -1 is for undefined.
    else await DotNet.invokeMethodAsync("PolarWolves-Server", `SwapTo${getCurrentPage()}WithReq`, selected.attr("id"), request);
}
...
async function CopySettingsFrom(e, game) {
    ...
    await DotNet.invokeMethodAsync("PolarWolves-Server", "CopySettingsFrom", steamId64, game);
}
...
async function RestoreSettingsTo(e, game) {
    ...
    await DotNet.invokeMethodAsync("PolarWolves-Server", "RestoreSettingsTo", steamId64, game);
}
...
async function BackupGameData(e, game) {
    ...
    await DotNet.invokeMethodAsync("PolarWolves-Server", "BackupGameData", steamId64, game);
}
...
async function ShowGameStatsSetup(e = null) {
    ...
    const enabledGames =
        await DotNet.invokeMethodAsync("PolarWolves-Server", "GetEnabledGames", currentPage, accountId);
    const disabledGames = await DotNet.invokeMethodAsync("PolarWolves-Server", "GetDisabledGames", currentPage, accountId);
...
async function toggleGameStats(safeGame, isChecked) {
    ...
    if (!isChecked) {
        ...
        await DotNet.invokeMethodAsync("PolarWolves-Server", `DisableGame`, game, accountId);
        ShowGameStatsSetup();
        return;
    }
...
async function showGameStatsVars(game) {
    ...
    // Checked: Get required variables and present to user.
    const required = await DotNet.invokeMethodAsync("PolarWolves-Server", `GetRequiredVars`, game);
    const existing = await DotNet.invokeMethodAsync("PolarWolves-Server", `GetExistingVars`, game, accountId);
    const hidden = await DotNet.invokeMethodAsync("PolarWolves-Server", `GetHiddenMetrics`, game, accountId);
    const globallyHidden = await DotNet.invokeMethodAsync("PolarWolves-Server", `GetGloballyHiddenMetrics`, game);
...
async function Modal_FinaliseGameVars(game, accountId) {
    ...
    const requiredVars = await DotNet.invokeMethodAsync("PolarWolves-Server", `GetRequiredVars`, game);
    const possibleHidden = await DotNet.invokeMethodAsync("PolarWolves-Server", `GetHiddenMetrics`, game, accountId);
    const globallyHidden = await DotNet.invokeMethodAsync("PolarWolves-Server", `GetGloballyHiddenMetrics`, game);
    ...
    $(".modalBG").fadeOut();
    const success = await DotNet.invokeMethodAsync("PolarWolves-Server", `SetGameVars`, currentPage, game, accountId, returnDict, hidden);
...
async function refreshAccount(game, accountId) {
    const currentPage = await getCurrentPageFullname();
    $(".modalBG").fadeOut();
    await DotNet.invokeMethodAsync("PolarWolves-Server", `RefreshAccount`, accountId, game, currentPage);
    location.reload();
}



Modal_FinalizeImage = async(dest) => await DotNet.invokeMethodAsync("PolarWolves-Server", `ImportNewImage`, JSON.stringify({ dest: dest, path: $("#FolderLocation").val() }));
...
async function openUserdata(e) {
    ...
    await DotNet.invokeMethodAsync("PolarWolves-Server", `SteamOpenUserdata`, selected.attr("id"));
}

async function showSteamOwnedGamesModal() {
    if (!getSelected()) return;

    const steamId = selected.attr("id");
    const selectedDisplayName = getSelectedDisplayName();

    $("#modalTitle").text("Account Games");
    $("#modal_contents").empty();
    $("#modal_contents").append(`<div class="steamOwnedGamesWindow steamOwnedGamesWindow--loading">
            <h3>${escapeHtml(selectedDisplayName || "Selected account")}</h3>
            <p class="steamOwnedGamesHint">Loading owned games...</p>
        </div>`);
    ...
    let result = null;
    try {
        const payload = await DotNet.invokeMethodAsync("PolarWolves-Server", "GetSteamOwnedGames", steamId);
        result = typeof payload === "string" ? JSON.parse(payload) : payload;
    } catch (err) {
        console.error(err);
        result = {
            success: false,
            message: "Failed to load account games.",
            games: [],
            totalCount: 0,
            accountDisplayName: selectedDisplayName
        };
    }

    const resultSuccess = (result.success ?? result.Success ?? false) === true;
    const resultMessage = result.message ?? result.Message ?? "";
    const resultGames = result.games ?? result.Games ?? [];
    const resultTotalCount = result.totalCount ?? result.TotalCount ?? resultGames.length;
    const resultAccountDisplayName = result.accountDisplayName ?? result.AccountDisplayName ?? "";

    const accountName = (resultAccountDisplayName && resultAccountDisplayName.trim() !== "")
        ? resultAccountDisplayName
        : selectedDisplayName;
    const games = Array.isArray(resultGames) ? resultGames : [];
    const totalCount = Number.isFinite(resultTotalCount) ? resultTotalCount : games.length;

    let content = `<div class="steamOwnedGamesWindow">
            <h3>${escapeHtml(accountName || "Selected account")}</h3>
            <div class="steamOwnedGamesSummary">${escapeHtml(`${totalCount} games found`)}</div>`;

    if (resultSuccess && resultMessage) {
        content += `<p class="steamOwnedGamesHint">${escapeHtml(resultMessage)}</p>`;
    }

    if (!resultSuccess) {
        content += `<p class="steamOwnedGamesHint">${escapeHtml(resultMessage || "Could not load this account's games.")}</p>`;
    } else if (games.length === 0) {
        content += `<p class="steamOwnedGamesHint">No games found for this account.</p>`;
    } else {
        content += `<div class="steamOwnedGamesList">`;
        for (let i = 0; i < games.length; i++) {
            const game = games[i];
            const appId = game.appId ?? game.AppId ?? "";
            const gameName = game.name ?? game.Name ?? appId;
            const installed = game.installedOnThisPc ?? game.InstalledOnThisPc ?? false;
            const playtimeMinutes = game.playtimeMinutes ?? game.PlaytimeMinutes ?? 0;
            const playtime = formatPlaytime(playtimeMinutes);
            const installedMarkup = installed ? `<span class="steamOwnedGamesTag">Installed</span>` : "";
            content += `<div class="steamOwnedGamesRow">
                    <div class="steamOwnedGamesName">${escapeHtml(gameName)}</div>
                    <div class="steamOwnedGamesMeta">
                        <span>#${escapeHtml(appId)}</span>
                        <span>${escapeHtml(playtime)}</span>
                        ${installedMarkup}
                    </div>
                </div>`;
        }
    ...
    content += `</div>`;
    $("#modal_contents").empty().append(content);
}
...
async function createShortcut(args = '') {
    ...
    await DotNet.invokeMethodAsync("PolarWolves-Server",
        "CreateShortcut",
        getCurrentPage(),
        accId,
        getDisplayName(),
...
// NEW LOGIN
newLogin = async() => await DotNet.invokeMethodAsync("PolarWolves-Server", `NewLogin_${getCurrentPage()}`);
hidePlatform = async() => await DotNet.invokeMethodAsync("PolarWolves-Server", "HidePlatform", selectedElem);
createPlatformShortcut = async() => await DotNet.invokeMethodAsync("PolarWolves-Server", "GiCreatePlatformShortcut", selectedElem);
...
async function exportAllAccounts() {
    ...
    const r = await DotNet.invokeMethodAsync("PolarWolves-Server", "GiExportAccountList", selectedElem);
...
// Link handling
OpenLinkInBrowser = async(link) => await DotNet.invokeMethodAsync("PolarWolves-Server", "OpenLinkInBrowser", link);
...
async function showModal(modaltype) {
    let input, platform;
    if (modaltype === "info") {
        const modalInfoVersion = await GetLang("Modal_Info_Version"),
        ...
        const discordServerUrl = "https://discord.gg/PLR";
        const githubProfileUrl = "https://github.com/PolarWoIF";

        $("#modalTitle").text(modalTitleInfo);
        $("#modal_contents").empty();

        const currentVersion = await DotNet.invokeMethodAsync("PolarWolves-Server", "GiGetVersion");
        $("#modal_contents").append(`<div class="infoWindow">
                <div class="imgDiv"><img width="100" margin="5" src="img/PolarWolves500.png" draggable="false" onclick="OpenLinkInBrowser('${discordServerUrl}');"></div>
                <div class="rightContent">
                    <h2>PolarWolves</h2>
                    <p>PolarWolves Team</p>
                    <div class="linksList">
                        <a onclick="OpenLinkInBrowser('${githubProfileUrl}');return false;" href=""><svg viewBox="0 0 24 24" draggable="false" alt="GitHub" class="modalIcoGitHub"><use href="img/icons/ico_github.svg#icoGithub"></use></svg>${modalInfoViewGitHub} @PolarWoIF</a>
                        <a onclick="OpenLinkInBrowser('${discordServerUrl}');return false;" href=""><svg viewBox="0 0 24 24" draggable="false" alt="Discord" class="modalIcoDiscord"><use href="img/icons/ico_discord.svg#icoDiscord"></use></svg>${modalInfoBugReport}</a>
                        <a onclick="OpenLinkInBrowser('${discordServerUrl}');return false;" href=""><svg viewBox="0 0 24 24" draggable="false" alt="Discord" class="modalIcoDiscord"><use href="img/icons/ico_discord.svg#icoDiscord"></use></svg>${modalInfoVisitSite}</a>
                        <a onclick="OpenLinkInBrowser('${discordServerUrl}');return false;" href=""><svg viewBox="0 0 2084 2084" draggable="false" alt="Info" class="modalIcoDoc"><use href="img/icons/ico_doc.svg#icoDoc"></use></svg>${modalInfoDisclaimer}</a>
                    </div>
                </div>
        ...
        if (platformName === "Basic") {
            extraButtons = await DotNet.invokeMethodAsync("PolarWolves-Server", "PlatformUserModalExtraButtons");
        ...
        const modalChangeUsername = await GetLang("Modal_SetPassword"),
            ...
            modalSetPasswordInfo = await GetLangSub("Modal_SetPassword_Info", { link: "https://discord.gg/PLR" });
        ...
        const currentBasicPlatformExe =
            await DotNet.invokeMethodAsync("PolarWolves-Server", "GiCurrentBasicPlatformExe", platform);
        ...
        const extraButtons = await DotNet.invokeMethodAsync("PolarWolves-Server", "PlatformUserModalExtraButtons");
        ...
        const modalText = await DotNet.invokeMethodAsync("PolarWolves-Server", "GetStatsString");
...
async function getLogicalDrives() {
    var folderContent = "";
    const logicalDrives = await DotNet.invokeMethodAsync("PolarWolves-Server", "GetLogicalDrives");
    folderContent = "<div>";
...
async function pathPickerClick(e) {
    ...
    if ($(e.target).hasClass("folder") && !$(e.target).hasClass("c")) {
        ...
        const fileSystemResult = await DotNet.invokeMethodAsync("PolarWolves-Server", getFunc, result);
...
//    DotNet.invokeMethodAsync("PolarWolves-Server", "GiUpdatePath", platformSettingsPath, $("#FolderLocation").val());
//    $(".modalBG").fadeOut();
...
async function Modal_Finalise(platform, platformSettingsPath) {
    ...
    await DotNet.invokeMethodAsync("PolarWolves-Server", "GiUpdatePath", platformSettingsPath, $("#FolderLocation").val());
    $(".modalBG").fadeOut();
...
async function Modal_Confirm(action, value) {
    ...
    const confirmAction = await DotNet.invokeMethodAsync("PolarWolves-Server", "GiConfirmAction", action, value);
...
async function Modal_FinaliseAccString(platform) {
    ...
    if (raw.indexOf(":{") === -1) {
        name = await DotNet.invokeMethodAsync("PolarWolves-Server", "GiGetCleanFilePath", raw);
    }
    ...
    await DotNet.invokeMethodAsync("PolarWolves-Server", platform + "AddCurrent", name);
    $(".modalBG").fadeOut();
    $("#acc_list").click();
}
...
async function Modal_FinaliseBackground() {
    ...
    await DotNet.invokeMethodAsync("PolarWolves-Server", "SetBackground", pathOrUrl);
    $(".modalBG").fadeOut();
}

async function Modal_FinaliseSwitcherPassword() {
    const switcherPassword = $("#SwitcherPassword").val();
    await DotNet.invokeMethodAsync("PolarWolves-Server", "SetSwitcherPassword", switcherPassword);
    $(".modalBG").fadeOut();
}
...
async function Modal_FinaliseUserDataFolder() {
    ...
    const pathOrUrl = $("#FolderLocation").val();
    await DotNet.invokeMethodAsync("PolarWolves-Server", "SetUserData", pathOrUrl);
    $(".modalBG").fadeOut();
}
...
async function Modal_FinaliseAccNameChange() {
	...
	const name = (raw.indexOf("POLARWOLVES:") === -1 ? raw.replace(/[<>: \.\"\/\\|?*]/g, "-") : raw); // Clean string if not a command string.
    await DotNet.invokeMethodAsync("PolarWolves-Server", "ChangeUsername", $(".acc:checked").attr("id"), name);
}

async function Modal_SaveNotes(accId) {
    await DotNet.invokeMethodAsync("PolarWolves-Server", `Set${getCurrentPage()}Notes`, accId, $('#accNotes').val());
    location.reload(true);
}
...
async function usernameModalCopyText() {
    ...
    const toastHintText = await DotNet.invokeMethodAsync("PolarWolves-Server", "PlatformHintText", platform);
    const code = await DotNet.invokeMethodAsync("PolarWolves-Server", "PlatformUserModalCopyText");

    await DotNet.invokeMethodAsync("PolarWolves-Server", "CopyToClipboard", code);
    window.notification.new({
	    type: "success",
        title: toastTitle,
...
async function serializeShortcuts() {
    ...
    if (getCurrentPage() === "Steam")
        await DotNet.invokeMethodAsync("PolarWolves-Server", "SaveShortcutOrderSteam", output);
    else
        await DotNet.invokeMethodAsync("PolarWolves-Server", "SaveShortcutOrder", output);
}
...
async function shortcut(action) {
    ...
    console.log(reqId);
    if (getCurrentPage() === "Steam")
        await DotNet.invokeMethodAsync("PolarWolves-Server", "HandleShortcutActionSteam", reqId, action);
    else
        await DotNet.invokeMethodAsync("PolarWolves-Server", "HandleShortcutAction", reqId, action);
    if (action === "hide") $(selectedElem).remove();
}

updateBarClick = async () => await DotNet.invokeMethodAsync("PolarWolves-Server", "UpdateNow");

async function initSavingHotKey() {
    hotkeys("ctrl+s", async function (event) {
        event.preventDefault();
        await DotNet.invokeMethodAsync("PolarWolves-Server", "GiCtrlS", getCurrentPage());
    });
}
...
async function highlightCurrentAccount(curAcc) {
    ...
    if (typeof animateAccountSelection === "function") {
        animateAccountSelection($(`#acc_list input[id='${curAcc}']`));
    }
...
restartAsAdmin = async(args = "") => await DotNet.invokeMethodAsync("PolarWolves-Server", "GiRestartAsAdmin", args);
