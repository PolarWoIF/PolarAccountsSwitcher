...
using PolarWolves_Server.Pages.General.Classes;
using PolarWolves_Server.Shared;
using PolarWolves_Server.Shared.ContextMenu;
using YamlDotNet.Serialization;
using YamlDotNet.Serialization.NamingConventions;

namespace PolarWolves_Server.Data
{
    public class AppSettings
    {
        ...
        [JsonProperty("ShareAnonymousStats", Order = 16)] private bool _statsShare = false;
        ...
        [JsonProperty("CheckForUpdates", Order = 24)] private bool _checkForUpdates = false;
        ...
        public static void CheckShortcuts()
        {
            ...
            DesktopShortcut = File.Exists(Path.Join(Shortcut.Desktop, "PolarWolves.lnk"));
            StartMenu = File.Exists(Path.Join(Shortcut.StartMenu, "PolarWolves.lnk"));
        ...
        private const string ProtocolScheme = "polarwolves";

        /// <summary>
        /// Check for existence of protocol key in registry (polarwolves:\\)
        /// </summary>
        [SupportedOSPlatform("windows")]
        private static bool Protocol_IsEnabled()
        {
            var key = Registry.ClassesRoot.OpenSubKey(ProtocolScheme);
        ...
        public static void Protocol_Toggle()
        {
            try
            {
                ...
                {
                    // Add
                    using var key = Registry.ClassesRoot.CreateSubKey(ProtocolScheme);
                    key?.SetValue("URL Protocol", "", RegistryValueKind.String);
                    using var defaultKey = Registry.ClassesRoot.CreateSubKey($@"{ProtocolScheme}\Shell\Open\Command");
                    defaultKey?.SetValue("", $"\"{Path.Join(Globals.AppDataFolder, "PolarWolves.exe")}\" \"%1\"", RegistryValueKind.String);
                    _ = GeneralInvocableFuncs.ShowToast("success", Lang["Toast_ProtocolEnabled"], Lang["Toast_ProtocolEnabledTitle"], "toastarea");
                }
                else
                {
                    // Remove
                    Registry.ClassesRoot.DeleteSubKeyTree(ProtocolScheme);
        ...
        public static void CheckForUpdate()
        {
            ...
            {
#if DEBUG
                var latestVersion = Globals.DownloadString("https://discord.gg/PLR/Projects/AccSwitcher/api?debug&v=" + Globals.Version);
#else
                var latestVersion = Globals.DownloadString("https://discord.gg/PLR/Projects/AccSwitcher/api?v=" + Globals.Version);
#endif
                var cleanedLatest = latestVersion.Replace("\r", "").Replace("\n", "");
                if (!DateTime.TryParseExact(cleanedLatest, "yyyy-MM-dd_mm", null, DateTimeStyles.None, out _))
                {
                    Globals.WriteToLog($"Update check ignored due to unexpected server response: '{cleanedLatest}'");
                    return;
                }
                if (CheckLatest(cleanedLatest)) return;
                // Show notification
                try
                {
            ...
            catch (Exception e) when (e is WebException or AggregateException)
            {
                ...
                Globals.WriteToLog(@"Could not reach https://discord.gg/PLR/ to check for updates.", e);
            }
        }
        ...
        public static void UpdateNow()
        {
            ...
            try
            {
                ...
                Globals.DownloadFile("https://discord.gg/PLR/Projects/AccSwitcher/latest/hashes.json", hashFilePath);
                ...
                foreach (var (key, value) in updaterDict)
                {
                    ...
                    if (File.Exists(key) && value == GeneralFuncs.GetFileMd5(key))
                        continue;
                    Globals.DownloadFile("https://discord.gg/PLR/Projects/AccSwitcher/latest/" + key.Replace('\\', '/'), key);
                }
        ...
        public static void StartUpdaterAsAdmin(string args = "")
        {
            ...
            var proc = new ProcessStartInfo
            {
                ...
                FileName = "updater\\PolarWolves-Updater.exe",
                Arguments = args,
        ...
        public static void AutoStartUpdaterAsAdmin(string args = "")
        {
            ...
            {
                _ = Process.Start(new ProcessStartInfo(Path.Join(Globals.AppDataFolder, @"updater\\PolarWolves-Updater.exe")) { UseShellExecute = true, Arguments = args });
                try
                {
                    AppData.ActiveNavMan.NavigateTo("EXIT_APP", true);
                }
