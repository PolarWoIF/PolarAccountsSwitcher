...
namespace PolarWolves_Server.Data
{
    public class AppStats
    {
        ...
        public static string GetStatsString()
        {
            ...
            GenerateTotals();
            SaveSettings();
            var returnString = $@"<h3>PolarWolves Stats - {DateTime.Now:yyyy-MM-dd HH:mm:ss}</h3><br>" +
                (AppSettings.StatsShare ? Lang["Stats_ShareEnabled", new { WebsiteLink = "OpenLinkInBrowser('https://discord.gg/PLR')" }] : Lang["Stats_ShareDisabled", new { WebsiteLink = "https://discord.gg/PLR" }]) +
                @$"<pre><br>
   ...
   - <b>{Lang["Stats_GamesLaunched"]}</b>{switcherStat.Value.GamesLaunched}<br>";
        ...
        public static void UploadStats()
        {
            try
            {
                ...
                var response = httpClient.PostAsync("https://discord.gg/PLR/Projects/AccSwitcher/api/stats/",
            ...
            } catch (Exception e)
            {
                ...
                Globals.WriteToLog(@"Could not reach https://discord.gg/PLR/ to upload statistics.", e);
            }
        }
