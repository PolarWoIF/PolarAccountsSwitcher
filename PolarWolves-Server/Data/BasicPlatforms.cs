...
namespace PolarWolves_Server.Data
{
        ...
        */
        ...
        public async static void UpdatePlatformsJson()
        {
            try
            {
                ...
                string latestPlatformsString = await client.GetStringAsync("https://raw.githubusercontent.com/PolarWolves/PolarWolves/master/PolarWolves-Server/Platforms.json?{currentDateTimeStamp}");
                ...
                using (JsonDocument document = JsonDocument.Parse(File.ReadAllText(PlatformsPathUser)))
                {
                    ...
                    foreach (var platform in root.GetProperty("Platforms").EnumerateObject())
                    {
                        ...
                        string platformImageUrl = $"https://raw.githubusercontent.com/PolarWolves/PolarWolves/master/PolarWolves-Server/wwwroot/img/platform/{platformImage}.svg";
    ...
    public sealed class CurrentPlatform
    {
        ...
        public void CurrentPlatformInit(string platform)
        {
            ...
            // Reset optional values to avoid carrying stale values from a previously selected platform.
            AllFilesRequired = false;
            HasRegistryFiles = false;
            PathListToClear = new List<string>();
            HasExtras = false;
            ShortcutFolders = new List<string>();
            ShortcutIgnore = new List<string>();
            ShortcutIncludeMainExe = true;
            SearchStartMenuForIcon = false;
            UsernameModalExtraButtons = "";
            UserModalCopyText = "";
            UserModalHintText = "";
            ...
            ProfilePicRegex = "";
            BackupPaths.Clear();
            BackupFileTypesIgnore = new List<string>();
            BackupFileTypesInclude = new List<string>();
            ClosingMethod = "Combined";
            StartingMethod = "Default";
            RegDeleteOnClear = false;
            GetPathFromShortcutNamed = "";
            UniqueIdFile = "";
            UniqueIdFolder = "";
            UniqueIdRegex = "";
            UniqueIdMethod = "";
            ExitBeforeInteract = false;
            ClearLoginCache = true;
            ...
            if (jPlatform.ContainsKey("AllFilesRequired")) AllFilesRequired = (bool)jPlatform["AllFilesRequired"];
