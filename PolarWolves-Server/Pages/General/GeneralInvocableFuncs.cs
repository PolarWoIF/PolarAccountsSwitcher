...
using BasicSettings = PolarWolves_Server.Data.Settings.Basic;
using SteamSettings = PolarWolves_Server.Data.Settings.Steam;

namespace PolarWolves_Server.Pages.General
{
    public class GeneralInvocableFuncs
    {
        ...
        {
            var fileName = "PolarWolves_main.exe";
            if (!AppData.PolarWolvesClientApp) fileName = Assembly.GetEntryAssembly()?.Location.Replace(".dll", ".exe") ?? "PolarWolves-Server_main.exe";
            else
            {
                // Is client app, but could be developing >> No _main just yet.
                if (!File.Exists(Path.Join(Globals.AppDataFolder, fileName)) && File.Exists(Path.Join(Globals.AppDataFolder, "PolarWolves.exe")))
                    fileName = Path.Combine(Globals.AppDataFolder, "PolarWolves.exe");
            }
        ...
        public static void OpenLinkInBrowser(string link)
        {
            ...
            {
                Globals.WriteToLog($"Blocked non-web link: {link}");
                return;
            }
        ...
        public static void CreateShortcut(string page, string accId, string accName, string args = "")
        {
            ...
            _ = s.Shortcut_Platform(
                ...
                $"Switch to {accName} [{platform}] in PolarWolves",
                true);
        ...
        public static string GiCrowdinList()
        {
            ...
            try
            {
                var html = new HttpClient().GetStringAsync(
                    "https://discord.gg/PLR/Projects/AccSwitcher/api/crowdin/").Result;
