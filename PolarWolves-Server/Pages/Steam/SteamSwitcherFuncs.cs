...
using PolarWolves_Server.Converters;
...
namespace PolarWolves_Server.Pages.Steam
{
    public class SteamSwitcherFuncs
    {
        ...
        public static void DownloadSteamAppsData()
        {
            ...
            try
            {
                ...
                // Below is the fix from https://discord.gg/PLR/issues/485 - Thanks https://github.com/crapier
                if (Globals.ReadWebUrl("https://api.polarwolves.local/sw/SteamAppList", out var appList))
            ...
            try
            {
                ...
                if (loginUsers["users"] != null)
                {
                    foreach (var jUsr in loginUsers["users"])
                    {
                        ...
                        {
                            Globals.WriteToLog("Could not import Steam user. Please send your loginusers.vdf file to PolarWolves Team for analysis.");
                        }
                    }
                }
            }
        ...
        public static async Task PrepareProfile(string steamId, bool noCache)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    httpClient.DefaultRequestHeaders.Add("User-Agent", "PolarWolves");
            ...
            if (!File.Exists(dlDir))
            {
                try
                {
                    if (!AppSettings.OfflineMode)
                    {
                        var downloaded = !string.IsNullOrWhiteSpace(su.ImageDownloadUrl) &&
                                         Globals.DownloadFile(su.ImageDownloadUrl, dlDir) &&
                                         File.Exists(dlDir);
                        su.ImgUrl = downloaded
        ...
        public static void SwapSteamAccounts(string steamId = "", int ePersonaState = -1, string args = "")
        {
            ...
            {
                var launchArgs = args;
                if (SteamSettings.StartSilent) launchArgs += " -silent";
                if (SteamSettings.OldUi) launchArgs += " -vgui";
        ...
        private static bool StartSteamWithFallbacks(string args)
        {
            if (TryStartSteamExecutable(SteamSettings.Exe(), args)) return true;

            foreach (var steamPath in GetSteamExecutableCandidates())
            {
                if (TryStartSteamExecutable(steamPath, args)) return true;
            }

            return TryStartSteamViaProtocol();
        }

        private static bool TryStartSteamExecutable(string steamPath, string args)
        {
            if (string.IsNullOrWhiteSpace(steamPath)) return false;

            steamPath = Environment.ExpandEnvironmentVariables(steamPath.Trim().Trim('"'));
            if (Directory.Exists(steamPath))
                steamPath = Path.Join(steamPath, "Steam.exe");

            if (!File.Exists(steamPath)) return false;
            if (!Globals.StartProgram(steamPath, SteamSettings.Admin, args, SteamSettings.StartingMethod)) return false;

            try
            {
                var folder = Path.GetDirectoryName(steamPath);
                if (!string.IsNullOrWhiteSpace(folder) &&
                    !folder.Equals(SteamSettings.FolderPath, StringComparison.OrdinalIgnoreCase))
                {
                    SteamSettings.FolderPath = folder;
                    SteamSettings.SaveSettings();
                }
            }
            catch (Exception e)
            {
                Globals.WriteToLog("Steam was started but failed to update stored Steam folder path.", e);
            }
        ...
        private static IEnumerable<string> GetSteamExecutableCandidates()
        {
            ...
            void Add(string path)
            {
                if (!string.IsNullOrWhiteSpace(path))
                    _ = candidates.Add(path);
            }

            Add(Registry.GetValue(@"HKEY_CURRENT_USER\Software\Valve\Steam", "SteamExe", null)?.ToString());
            Add(Registry.GetValue(@"HKEY_CURRENT_USER\Software\Valve\Steam", "SteamPath", null)?.ToString());
            Add(Registry.GetValue(@"HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Valve\Steam", "InstallPath", null)?.ToString());
            Add(Registry.GetValue(@"HKEY_LOCAL_MACHINE\SOFTWARE\Valve\Steam", "InstallPath", null)?.ToString());
            Add(ExtractExePathFromCommand(Registry.GetValue(@"HKEY_CLASSES_ROOT\steam\Shell\Open\Command", "", null)?.ToString()));

            Add(Path.Join(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFilesX86), "Steam"));
            Add(Path.Join(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "Steam"));
            Add(Path.Join(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Programs", "Steam"));
        ...
        private static bool TryStartSteamViaProtocol()
        {
            try
            {
            ...
            {
                Globals.WriteToLog("Failed to start Steam through URI protocol steam://open/main.", e);
                return false;
            }
        }

        private static void BringSteamToFrontSoon()
        {
            _ = Task.Run(async () =>
            {
                try
                ...
                {
                    Globals.WriteToLog("Steam started, but failed to bring the main Steam window to the front.", e);
                }
            });
        }
        ...
        public static void SaveSteamUsersIntoVdf(List<Index.Steamuser> userAccounts)
        {
            ...
            {
                Globals.WriteToLog("Failed to swap Steam users! Could not create temp loginusers.vdf file, and replace original using workaround! Contact PolarWolves Team.", ex);
        ...
        private class SteamOwnedGame
        {
            public string AppId { get; set; } = "";
            public string Name { get; set; } = "";
            public int PlaytimeMinutes { get; set; }
            public bool InstalledOnThisPc { get; set; }
        }
        ...
        private static bool TryPopulateOwnedGamesFromLocalConfig(string steamId, SteamOwnedGamesResponse response)
        {
            try
            {
                var steamId32 = new SteamIdConvert(steamId).Id32;
                var localConfigPath = Path.Join(SteamSettings.FolderPath, "userdata", steamId32, "config", "localconfig.vdf");
                if (!File.Exists(localConfigPath)) return false;

                var vdfToken = VdfConvert.Deserialize(Globals.ReadAllText(localConfigPath));
                var root = new JObject { vdfToken.ToJson() };
                var userStore = root.GetValue("UserLocalConfigStore", StringComparison.OrdinalIgnoreCase) as JObject;
                if (userStore == null) return false;

                var gamePlaytimeMap = new Dictionary<string, int>();

                static JObject TryGetObject(JObject parent, string key) =>
                    parent?.GetValue(key, StringComparison.OrdinalIgnoreCase) as JObject;

                // Played apps history with playtime data.
                var softwareObj = TryGetObject(userStore, "Software");
                var valveObj = TryGetObject(softwareObj, "Valve");
                var steamObj = TryGetObject(valveObj, "Steam");
                var steamAppsObj = TryGetObject(steamObj, "apps");
                if (steamAppsObj != null)
                {
                    foreach (var appProp in steamAppsObj.Properties())
                    {
                        if (!appProp.Name.All(char.IsDigit)) continue;

                        var playtime = (appProp.Value as JObject)?.GetValue("Playtime", StringComparison.OrdinalIgnoreCase)?.Value<int?>() ?? 0;
                        if (!gamePlaytimeMap.TryGetValue(appProp.Name, out var existingPlaytime) || playtime > existingPlaytime)
                            gamePlaytimeMap[appProp.Name] = playtime;
                    }
                }

                // UserAppConfig contains many app IDs tied to this account (often broader than played list).
                var userAppConfigObj = TryGetObject(userStore, "UserAppConfig");
                if (userAppConfigObj != null)
                {
                    foreach (var appProp in userAppConfigObj.Properties())
                    {
                        if (!appProp.Name.All(char.IsDigit)) continue;
                        if (!gamePlaytimeMap.ContainsKey(appProp.Name))
                            gamePlaytimeMap[appProp.Name] = 0;
                    }
                }

                // Some builds also store app IDs in a root-level apps object.
                var rootAppsObj = TryGetObject(userStore, "apps");
                if (rootAppsObj != null)
                {
                    foreach (var appProp in rootAppsObj.Properties())
                    {
                        if (!appProp.Name.All(char.IsDigit)) continue;
                        if (!gamePlaytimeMap.ContainsKey(appProp.Name))
                            gamePlaytimeMap[appProp.Name] = 0;
                ...
                if (gamePlaytimeMap.Count == 0) return false;

                var installedGameIds = new HashSet<string>(SteamSettings.InstalledGames.Value);
                response.Games = gamePlaytimeMap
                    .Select(kvp =>
                    {
                        var appId = kvp.Key;
                        var appName = SteamSettings.AppIds.Value.GetValueOrDefault(appId) ?? appId;
                        return new SteamOwnedGame
                        {
                            AppId = appId,
                            Name = appName,
                            PlaytimeMinutes = kvp.Value,
                            InstalledOnThisPc = installedGameIds.Contains(appId)
                        };
                    })
                    .OrderBy(g => g.Name, StringComparer.OrdinalIgnoreCase)
                    .ToList();

                response.TotalCount = response.Games.Count;
                response.Success = true;
                response.ErrorCode = "local_fallback";
                response.Message = "Loaded from local Steam account data. Add Steam Web API key for a more complete library list.";
                return true;
            }
            catch (Exception ex)
            {
                Globals.WriteToLog("Failed to load Steam owned games from local config.", ex);
                return false;
            }
        }

        [JSInvokable]
        public static Task<string> GetSteamOwnedGames(string steamId)
        {
            var response = new SteamOwnedGamesResponse
            {
                SteamId = steamId
            };
            ...
            try
            {
                ...
                {
                    response.ErrorCode = "invalid_steam_id";
                    response.Message = Lang["Toast_NoValidSteamId"];
                    return Task.FromResult(JsonConvert.SerializeObject(response));
                }

                var su = AppData.SteamUsers.FirstOrDefault(x => x.SteamId == steamId);
                response.AccountDisplayName = su == null ? steamId : GetName(su);

                if (AppSettings.OfflineMode)
                {
                    if (TryPopulateOwnedGamesFromLocalConfig(steamId, response))
                        return Task.FromResult(JsonConvert.SerializeObject(response));

                    response.ErrorCode = "offline_mode";
                    response.Message = "Owned games lookup is unavailable in offline mode.";
                    return Task.FromResult(JsonConvert.SerializeObject(response));
                }

                if (string.IsNullOrWhiteSpace(SteamSettings.SteamWebApiKey))
                {
                    if (TryPopulateOwnedGamesFromLocalConfig(steamId, response))
                        return Task.FromResult(JsonConvert.SerializeObject(response));

                    response.ErrorCode = "missing_api_key";
                    response.Message = "Steam Web API key is missing. Add it in Steam settings to load account games.";
                    return Task.FromResult(JsonConvert.SerializeObject(response));
                }

                var uri =
                    $"https://api.steampowered.com/IPlayerService/GetOwnedGames/v1/?key={WebUtility.UrlEncode(SteamSettings.SteamWebApiKey)}&steamid={steamId}&include_appinfo=true&include_played_free_games=true";
                if (!Globals.ReadWebUrl(uri, out var jsonString) || string.IsNullOrWhiteSpace(jsonString))
                {
                    if (TryPopulateOwnedGamesFromLocalConfig(steamId, response))
                        return Task.FromResult(JsonConvert.SerializeObject(response));

                    response.ErrorCode = "request_failed";
                    response.Message = "Failed to query Steam owned games for this account.";
                    return Task.FromResult(JsonConvert.SerializeObject(response));
                }

                if (!IsSteamApiKeyValid(jsonString))
                {
                    response.ErrorCode = "invalid_api_key";
                    response.Message = Lang["Toast_SteamWebKeyInvalid"];
                    return Task.FromResult(JsonConvert.SerializeObject(response));
                }

                var payload = JObject.Parse(jsonString);
                var result = payload["response"] as JObject;
                if (result == null || !result.Properties().Any())
                {
                    if (TryPopulateOwnedGamesFromLocalConfig(steamId, response))
                        return Task.FromResult(JsonConvert.SerializeObject(response));

                    response.ErrorCode = "private_profile";
                    response.Message = "This account library is private, so Steam did not return game ownership data.";
                    return Task.FromResult(JsonConvert.SerializeObject(response));
                }

                var installedGameIds = new HashSet<string>(SteamSettings.InstalledGames.Value);
                var games = result["games"] as JArray ?? new JArray();
                ...
                foreach (var game in games)
                {
                    ...
                    {
                        gameName = SteamSettings.AppIds.Value.GetValueOrDefault(appId) ?? appId;
                    }

                    response.Games.Add(new SteamOwnedGame
                    {
                        AppId = appId,
                        Name = gameName,
                        PlaytimeMinutes = game["playtime_forever"]?.Value<int>() ?? 0,
                        InstalledOnThisPc = installedGameIds.Contains(appId)
                    });
                }
                ...
                response.Games = response.Games
                    .OrderBy(g => g.Name, StringComparer.OrdinalIgnoreCase)
                    .ToList();
                response.TotalCount = result["game_count"]?.Value<int>() ?? response.Games.Count;
                response.Success = true;
                return Task.FromResult(JsonConvert.SerializeObject(response));
            }
            catch (Exception ex)
            {
                Globals.WriteToLog("Failed to load Steam owned games.", ex);
                response.ErrorCode = "exception";
                response.Message = "Failed to load account games. See logs for details.";
                return Task.FromResult(JsonConvert.SerializeObject(response));
            }
        }
