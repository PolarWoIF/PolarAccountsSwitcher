...
namespace PolarWolves_Server.Pages.Basic
{
    public class BasicSwitcherFuncs
    {
            ...
            if (OperatingSystem.IsWindows() && accFile.StartsWith("REG:"))
            {
                ...
                switch (res)
                {
                    ...
                    default:
                        ...
                        Globals.WriteToLog("Check to see what is expected here and report to PolarWolves Team.");
                        return res;
                }
            }
            ...
            foreach (var (accFile, savedFile) in CurrentPlatform.LoginFiles)
            {
                ...
                if (accFile.StartsWith("REG:"))
                {
                    ...
                    if (ReadRegistryKeyWithErrors(trimmedName, out var response, CurrentPlatform.AllFilesRequired)) // Remove "REG:                    " and read data
                    {
                        ...
                        else Globals.WriteToLog($"Unexpected registry type encountered (2)! Report to PolarWolves Team. {response.GetType()}");
                    }
                    continue;
                }
        ...
        /// <param name="reverse">FALSE: Platform -> LoginCache. TRUE: LoginCache -> Jâ€¢â€¢Platform</param>
        ...
        public static string GetUniqueId()
        {
            if (OperatingSystem.IsWindows() && CurrentPlatform.UniqueIdMethod is "REGKEY" && !string.IsNullOrEmpty(CurrentPlatform.UniqueIdFile))
            {
                ...
                switch (r)
                {
                    ...
                    default:
                        Globals.WriteToLog($"Unexpected registry type encountered (1)! Report to PolarWolves Team. {r.GetType()}");
                        return "";
                }
            }
