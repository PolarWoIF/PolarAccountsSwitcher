...
void launch_polarwolves_program(const char* arg)
{
...
int finalizeUpdate(std::string sfrom, std::string sto) {
	SetConsoleTitle(_T("PolarWolves - Finalizing Update..."));
	...
	// Launch the main program from sto/PolarWolves.exe in sto
	std::string exe_name = "PolarWolves.exe";
...
int main(int argc, char* argv[])
{
	...
	SetConsoleTitle(_T("PolarWolves - Runtime installer"));
	cout << "Welcome to the PolarWolves - Runtime installer" << endl <<
		"------------------------------------------------------------------------" << endl << endl;
	...
	if (argc > 1)
	{
		...
		if (args_contain("net", argc, argv))
		{
			...
			verify_net();
			if (argc > 2)
				launch_polarwolves_program(argv[argc - 1]);
			exit(1);
		}
	...
	cout << "You can close this window to stop now. Detailed instructions to install these" << endl <<
			...
			"https://discord.gg/PLR/wiki#1-installing-required-runtimes" << endl << endl;
	...
	cout << "This script is provided AS-IS without any warranties of any kind" << endl <<
			"Please also see the PolarWolves's Privacy Policy:" << endl << 
			"https://discord.gg/PLR/blob/master/PRIVACY.md" << endl << endl;
	...
	// Launch main program:
	if (!args_contain("nostart", argc, argv)) {
		string exe_name = "PolarWolves.exe";
